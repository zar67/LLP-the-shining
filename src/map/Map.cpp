...
bool Map::generateRooms()
{
    ...
    rooms[2][2] = Room(num_initialised_rooms, "data/Rooms/NESW.png", 2, 2, true, true, true, true);
    ...
    while (!rooms_to_generate.empty())
    {
        ...
        if (rooms[x][y].getId() == -1)
        {
            ...
            std::string doors_needed = needNorthDoor(x,y) +
                                       needEastDoor(x,y) +
                                       needSouthDoor(x,y) +
                                       needWestDoor(x,y);
            ...
            // For Each File
            std::vector<std::string> files = ASGE::FILEIO::enumerateFiles("data/Rooms");
            for (auto name : files)
            {
    ...
    for (int i = 0; i < 5; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            std::cout << i << ", " << j << " : " << rooms[i][j].getId() << "->" << rooms[i][j].getNorth() << ", " << rooms[i][j].getEast() << ", " << rooms[i][j].getSouth() << ", " << rooms[i][j].getWest() << std::endl;
        }
        std::cout << std::endl;
    }
